...
const cinematicPerformanceModeStorageKey = "deltavPerformanceMode";
const cinematicPerformanceModes: readonly CinematicPerformanceMode[] = [
  "full",
  "reduced",
  "minimal"
];
...
const gameMenuOpeningCameraPose = {
  // This is a static heliocentric shot: the camera looks past the Sun rather
  // than following a planet. That keeps the Sun fixed on screen while the
  // bodies advance around it during the menu demo.
  focus: [-854.7468346470079, 0, -111.46349773307867],
  yaw: -2.5743606466916362,
  pitch: 0.9100000000000004,
  distance: 5016.91343231237,
  focusedTargetKey: null,
  trackedFocusTargetKey: null,
  displayScaleFocusTargetKey: "node:moon_node",
  displayScaleDistance: 6352.573497655849
} as const;
...
const gameMenuCrtFlickerMinDelayMs = 2_800;
const gameMenuCrtFlickerMaxDelayMs = 7_000;
...
export async function createDeltaVApp(root: HTMLElement): Promise<void> {
  ...
  const reducedBloomResolutionButton = createDebugModeButton("BLOOM HIGH 40%");
  ...
  let isCommandConsoleAnimatingLiveRows = false;
  let shouldRefreshCommandConsoleAfterLiveUpdate = false;
  let hasRenderedLiveCommandRows = false;
  ...
  let reducedBloomResolutionMode: "on" | "off" = "off";
  ...
  let debugAiLevel: AiPlanningLevel = 3;
  ...
  const tutorialFirstEnemyKillReplayCueFocusBeforeRewindMs = 320;
  ...
  let previousLiveDynamicCommandRows: readonly CommandConsoleRow[] = [];
  ...
  const proceduralBatchMapCycle = createProceduralBatchMapCycle();
  ...
  function updateStatus(): void {
    ...
    status.textContent = `Turn ${snapshot.turn} | ΔV ${playerDv}${projectedDv} | ${snapshot.nodes.length} active nodes | ${selectedMapPreset.statusLabel}${burnState}${fireState}${launchState}${matchState}`;
  ...
  function startGameMenuDemo(): void {
    ...
    gameMenuScreen = "main";
    gameMenuNewGameOpponentCount = 1;
    ...
    gameMenuOrbitEpochTurn = currentOrbitTurn;
    content = shiftSolarSystemOrbitPhase(gameMenuBaseContent, gameMenuOrbitEpochTurn);
  ...
  function createGameMenuDemoInitialState(): GameState {
    const nextState = createInitialAiStateForGameMode("3p");
    const controllerOverrides = createControllerAuditOverrides(nextState);

    return appendStartStateAudit(nextState, { controllerOverrides });
  }
  ...
  function renderGameMenu(): void {
    gameMenu.classList.toggle("is-hidden", !isGameMenuDemoActive);
    commandConsole.classList.toggle("is-hidden", isGameMenuDemoActive);
  ...
  function appendGameMenuNewGameOptions(
    ...
    const factionsAction = createGameMenuAction(
      getGameMenuFactionLabel(),
      (action) => {
        gameMenuNewGameOpponentCount = gameMenuNewGameOpponentCount === 1 ? 2 : 1;
        typeGameMenuAction(action, getGameMenuFactionLabel());
      },
      { tone: "soft" },
      typingTargets
    );
  ...
  function appendGameMenuOptions(
    ...
    const bloomAction = createGameMenuAction(
      ...
      (action) => {
        ...
        reducedBloomResolutionMode = nextBloomMode === "low" ? "on" : "off";
        cinematicRenderer?.setBloomEnabled(uiBloomMode === "on");
        cinematicRenderer?.setUiBloomEnabled(uiBloomMode === "on");
        cinematicRenderer?.setReducedBloomResolutionEnabled(reducedBloomResolutionMode === "on");
        updateCommandConsoleModeControls();
        redraw();
    ...
    const performanceAction = createGameMenuAction(
      getGameMenuPerformanceLabel(),
      (action) => {
        const currentMode = getCinematicPerformanceMode();
        const currentIndex = cinematicPerformanceModes.indexOf(currentMode);
        const nextMode =
          cinematicPerformanceModes[(currentIndex + 1) % cinematicPerformanceModes.length] ??
          "auto";
        window.localStorage.setItem(cinematicPerformanceModeStorageKey, nextMode);
        redraw();
        typeGameMenuAction(action, getGameMenuPerformanceLabel());
      },
      { tone: getCinematicPerformanceMode() === "full" ? "bright" : "regular" },
      typingTargets
    );
  ...
  async function typeGameMenuTargetsSequentially(
    ...
    for (const target of targets) {
      if (generation !== gameMenuTypingGeneration || !isGameMenuDemoActive) {
        return;
      }
  ...
  function startConfiguredGameFromMenu(): void {
    const mode: MultiFactionGameModeId = gameMenuNewGameOpponentCount === 1 ? "2p" : "3p";
    ...
    const waitForCurrentTurn = (): void => {
      ...
      planningTimerMode = "auto";
      planningTimerDurationOverrideMs = timerDurationMs;
      hasConsumedZeroTimerInitialCountdown = true;
      clearZeroTimerAutoRestart();

      if (selectedMapPreset.procedural === true) {
        resetNewGameWithAutomaticProceduralMap(mode);
      } else {
  ...
  function resetNewGameWithAutomaticProceduralMap(mode: MultiFactionGameModeId): void {
    ...
    installAutomaticProceduralMap(mode, automaticMap, false, "new-game");
  }

  function resetAiBatchWithAutomaticProceduralMap(mode: MultiFactionGameModeId): void {
    const automaticMap = proceduralBatchMapCycle.createNext(selectedMapPreset);
  ...
  function getGameMenuFactionLabel(): string {
    return gameMenuNewGameOpponentCount === 1 ? "2 FACTIONS" : "3 FACTIONS";
  }
  ...
  function getGameMenuBloomMode(): "high" | "low" | "off" {
    ...
    return reducedBloomResolutionMode === "on" ? "low" : "high";
  }

  function getGameMenuPerformanceLabel(): string {
    const mode = getCinematicPerformanceMode();
    ...
    if (mode === "minimal") {
      return "PERFORMANCE COMPATIBILITY";
    }
  ...
  function updateCommandConsoleModeControls(): void {
    ...
    reducedBloomResolutionButton.textContent =
      reducedBloomResolutionMode === "on" ? "BLOOM LOW 20%" : "BLOOM HIGH 40%";
    ...
    reducedBloomResolutionButton.classList.toggle("is-active", reducedBloomResolutionMode === "on");
    ...
    reducedBloomResolutionButton.setAttribute(
      "aria-pressed",
      reducedBloomResolutionMode === "on" ? "true" : "false"
    );
  ...
  function startMusicOnGameStart(): void {
    if (!musicEngine.isSupported || !isMusicEnabled) {
      if (!isMusicEnabled) {
        cancelMusicAutoplayUnlock();
        musicEngine.stop();
      }
  ...
  function updateDebugPanel(): void {
    ...
    const proceduralDebugText = formatProceduralMapDebug(currentProceduralDebug, state.gameMode);
    ...
    debugPanel.textContent = [
      ...
      ...recordingMessages,
      ...debugMessages,
      ...(proceduralDebugText === "" ? [] : ["", proceduralDebugText])
    ].join("\n");
  }
  ...
  function updateCommandConsole(): void {
    ...
    if (isCommandConsoleTypingLiveBlock || isCommandConsoleAnimatingLiveRows) {
    ...
    if (shouldTypeNextLiveCommandBlock) {
      ...
      void appendLiveCommandBlockSequential(rows, { typewriter: true })
        .then(() => {
          hasRenderedLiveCommandRows = true;
    ...
    if (shouldAnimateLiveDynamicCommandRows(rows)) {
      isCommandConsoleAnimatingLiveRows = true;
      void renderLiveCommandRowsWithDynamicAnimation(rows).finally(() => {
        isCommandConsoleAnimatingLiveRows = false;
        flushQueuedCommandConsoleRefresh();
      });
      return;
    }
  ...
  function shouldAnimateLiveDynamicCommandRows(rows: readonly CommandConsoleRow[]): boolean {
    if (!hasRenderedLiveCommandRows) {
      return false;
    }

    return (
      getCommandRowsSignature(getDynamicCommandRows(rows)) !==
      getCommandRowsSignature(previousLiveDynamicCommandRows)
    );
  }
  ...
  async function renderLiveCommandRowsWithDynamicAnimation(
    ...
    prepareCommandLiveRowsBlock();
    appendExecutePromptToLiveBlock();
    const previousRows = previousLiveDynamicCommandRows;
    const previousRowsByKey = new Map(previousRows.map((row) => [row.key, row]));
    const nextRowsByKey = new Map(getDynamicCommandRows(rows).map((row) => [row.key, row]));
    const pendingEraseAnimations: Promise<void>[] = [];
    ...
    for (const row of rows) {
      ...
      const isNewRow = !previousRowsByKey.has(row.key);
      const line = appendCommandConsoleLine(commandLiveRows, row.parts, row.className, {
        metadata: row.metadata,
        typewriter: isNewRow && shouldTypewriteLiveCommandRow(row)
      });
      scrollCommandTranscriptToEnd();

      if (isNewRow && line.typewriterDone !== undefined) {
        await line.typewriterDone;
        scrollCommandTranscriptToEnd();
      }
    }

    for (const row of previousRows) {
      if (row.key === undefined || nextRowsByKey.has(row.key)) {
        continue;
      }

      const line = appendCommandConsoleLine(
        commandLiveRows,
        row.parts,
        mergeCommandConsoleClassNames(row.className, "command-console__line--erasing"),
        {
          typewriter: false
        }
      );
      pendingEraseAnimations.push(eraseCommandConsoleLine(line));
    }

    await Promise.all(pendingEraseAnimations);
    hasRenderedLiveCommandRows = true;
    previousLiveDynamicCommandRows = getDynamicCommandRows(rows);
    scrollCommandTranscriptToEnd();
  }
  ...
  async function appendResolutionTranscriptRows(fromDebugEventIndex: number): Promise<void> {
    ...
    await appendCommandTimelineEntriesSequential(getVisibleCommandTimelineEntries(entries), {
      typewriter: true
    });
    scrollCommandTranscriptToEnd();
  }
  ...
  function getDynamicCommandRows(rows: readonly CommandConsoleRow[]): readonly CommandConsoleRow[] {
    return rows.filter((row) => row.key !== undefined);
  }
  ...
  function getCommandRowsSignature(rows: readonly CommandConsoleRow[]): string {
    return rows
      .map((row) => {
        return `${row.key ?? ""}:${getCommandConsolePartsText(row.parts)}`;
      })
      .join("\n");
  }

  function getCommandConsolePartsText(parts: readonly CommandConsolePart[]): string {
    return parts
      .map((part) => {
        return part.text ?? part.element?.textContent ?? "";
      })
      .join("");
  }

  function mergeCommandConsoleClassNames(first: string | undefined, second: string): string {
    ...
    for (const part of parts) {
      ...
      if (part.text !== undefined) {
        if (options.typewriter === true) {
          span.textContent = "";
          typewriterTargets.push({ span, text: part.text });
        } else {
  ...
  function eraseCommandConsoleLine(line: CommandConsoleLineElement): Promise<void> {
    const reservedHeight = Math.ceil(line.getBoundingClientRect().height);

    if (reservedHeight > 0) {
      line.style.height = `${reservedHeight}px`;
    }

    const targets = [...line.querySelectorAll<HTMLSpanElement>("span")]
      .filter((span) => {
        return span.className !== "command-console__type-cursor";
      })
      .map((span) => {
        return { span, text: span.textContent ?? "" };
      });
    const totalCharacters = targets.reduce((total, target) => {
      return total + target.text.length;
    }, 0);
    const durationMs = clampNumber(totalCharacters * 1.1, 35, 90);
    const startedAt = performance.now();

    const renderRemainingCharacters = (visibleCharacters: number) => {
      let remainingCharacters = visibleCharacters;
    ...
    return new Promise((resolve) => {
      ...
      const eraseNextFrame = () => {
        const progress = clampNumber((performance.now() - startedAt) / durationMs, 0, 1);
        const visibleCharacters = Math.max(0, Math.ceil(totalCharacters * (1 - progress)));
        renderRemainingCharacters(visibleCharacters);
        ...
        window.requestAnimationFrame(eraseNextFrame);
      };
  ...
  function maybeShowPostMatchReport(): void {
    ...
    const outcome = detectPostMatchOutcome(content, state, snapshot);
    ...
    lastVictoryAudit = createVictoryAudit(content, state, snapshot, outcome);
    ...
    const victoryContradictions = createVictoryAuditContradictions(
      content,
      state,
      snapshot,
      outcome,
    ...
    console.info("VICTORY_AUDIT", lastVictoryAudit);
    console.info("MAP_OUTCOME_AUDIT", lastMapOutcomeAudit);
    console.info("VICTORY_CONFIRMED", {
      winner: outcome.winner,
      reason: lastVictoryAudit.reason,
      survivingTritiumPath: lastVictoryAudit.factionStates[outcome.winner]?.countedTritium ?? [],
  ...
  function showTutorialCompletionReport(): void {
    showPostMatchReport(
      [
        "TUTORIAL COMPLETE",
        "",
  ...
  async function restartZeroTimerAutorunMatch(): Promise<void> {
    ...
    try {
      if (restartPreset.procedural === true) {
        const batchMap = proceduralBatchMapCycle.createNext(restartPreset);
        ...
        hasConsumedZeroTimerInitialCountdown = true;
        resetRuntimeAfterGameReset({ preserveCamera: true });
        return;
      }
  ...
  function logVictoryDelayAudit(extra: Readonly<Record<string, unknown>> | null = null): void {
    const audit = createVictoryDelayAudit(content, state, snapshot);
  ...
  function playReplayFromCommandRow(
    ...
    if (commandLogTimeReviewState === null) {
      void rewindCommandLogToEvent(targetId, focusTargetKeys, commandRowKey);
      return;
    }
  ...
  function animateCommandLogTimeReviewToPosition(
    ...
    if (clampedTarget > startPosition + 0.001) {
      return playCommandLogReviewForwardToPosition(
        clampedTarget,
        targetEventId,
        focusTargetKeys,
        activeCommandRowKey
    ...
    const durationMs = isRewind
      ? getAdaptiveRewindDurationMs(distance)
      : clampNumber(distance * commandLogTimeReviewDurations.replayTurnMs, 120, 6200);
    const startedAt = performance.now();
    ...
    return new Promise((resolve) => {
      ...
      const tick = () => {
        const linearProgress =
          durationMs <= 0 ? 1 : clampNumber((performance.now() - startedAt) / durationMs, 0, 1);
        const positionProgress = isRewind
          ? easeAdaptiveRewindProgress(linearProgress)
          : linearProgress;
        const position = startPosition + (clampedTarget - startPosition) * positionProgress;
  ...
  async function playCommandLogReviewForwardToPosition(
    ...
    const startTransitionIndex = clampNumber(
      Math.floor(reviewState.currentPosition),
      0,
      Math.max(0, replayTape.transitions.length - 1)
    );
    const endTransitionExclusive = clampNumber(
      Math.ceil(clampedTarget),
      0,
      replayTape.transitions.length
    );
    ...
    try {
      for (let index = startTransitionIndex; index < endTransitionExclusive; index += 1) {
        if (replayCancelRequested || commandLogTimeReviewState !== reviewState) {
          return;
        }

        const transition = replayTape.transitions[index];

        if (transition === undefined) {
          break;
        }

        const eventId = getCommandLogEventIdNearReviewPosition(index) ?? targetEventId;
        reviewState.currentPosition = index;
        reviewState.transitionIndex = index;
        ...
        await cinematicRenderer.animateReplayTransition(
          transition.from,
          transition.to,
          commandLogTimeReviewDurations.replayTurnMs
        );
        reviewState.currentPosition = index + 1;
        reviewState.transitionIndex = Math.min(
          index + 1,
          Math.max(0, replayTape.transitions.length - 1)
        );
  ...
  function syncLogReviewStaticFocusTargetKeys(targetKeys: readonly string[]): void {
    ...
    const focusedTargetKey = cinematicRenderer.focusFirstAvailableTargetWithoutTracking(targetKeys);
  ...
  function getReplayPositionForResolutionEventId(eventId: string): number | null {
    return getReplayTransitionIndexForResolutionEventId(eventId);
  }
  ...
  function getCommandLogEventIdNearReviewPosition(position: number): string | null {
    ...
    const transition = replayTape.transitions[transitionIndex];
    return (
      transition?.entries.find((entry) => entry.logLink !== undefined)?.logLink
  ...
  function getCinematicPerformanceMode(): CinematicPerformanceMode {
    const storedMode = window.localStorage.getItem(cinematicPerformanceModeStorageKey);

    // AUTO used frame cadence as a proxy for render cost. Safari's WebGL presenter can settle at
    // 30 Hz while the renderer itself is comfortably below 16.7 ms, causing AUTO to lower detail
    // and resolution without improving presentation. Preserve explicit compatibility choices,
    // but migrate AUTO and new installs to the full-quality path.
  ...
  function ensureCinematicRenderer(): void {
    ...
    cinematicRenderer.setCameraInputEnabled(!isGameMenuDemoActive);
  ...
  function selectTutorialTarget(target: string): void {
    const mandatoryLaunch = getNextPlayerMandatoryLaunch();
  ...
  function updateInteractionLocks(): void {
    ...
    gameMenuModeButton.disabled =
      isGameMenuDemoActive || isReplayMode || isMandatoryLaunchLocked || isTurnTransitionActive;
  ...
  async function executeCurrentTurn(): Promise<void> {
    ...
    const shouldContinueTutorialShipyardFireQueued =
      tutorialState?.phase === "shipyardFireQueued" && !isManualExecutePromptDisabled();
    ...
    const shouldAutoAdvanceTutorialShipyardContestedFire =
      tutorialState?.phase === "shipyardContestedFireQueued" && !isManualExecutePromptDisabled();
  ...
  reducedBloomResolutionButton.addEventListener("click", () => {
    reducedBloomResolutionMode = reducedBloomResolutionMode === "on" ? "off" : "on";
    cinematicRenderer?.setReducedBloomResolutionEnabled(reducedBloomResolutionMode === "on");
    appendDebugPanelMessage(
      `BLOOM RESOLUTION ${reducedBloomResolutionMode === "on" ? "LOW 20%" : "HIGH 40%"}`
    );
    updateCommandConsoleModeControls();
    redraw();
  });
  ...
  function handleCommandLiveClick(event: MouseEvent): void {
    ...
    event.preventDefault();
    void playCommandLogReviewForwardToPosition(replayTape.transitions.length, null);
  }
  ...
  function handleCommandLogClick(event: MouseEvent): void {
    ...
    event.preventDefault();
    event.stopPropagation();
    playReplayFromCommandRow(targetId, focusCommandScrollbackLineTarget(line), commandRowKey);
  }
  ...
  async function playTutorialFirstEnemyKillReplayCue(line: HTMLElement | null): Promise<void> {
    ...
    try {
      ...
      if (focusTargetKeys.length > 0) {
        await waitForTutorialFirstEnemyKillReplayCueFocusBeforeRewind();
      }
  ...
  function waitForTutorialFirstEnemyKillReplayCueFocusBeforeRewind(): Promise<void> {
    return new Promise((resolve) => {
      window.setTimeout(resolve, tutorialFirstEnemyKillReplayCueFocusBeforeRewindMs);
    });
  }

  function showTutorialFirstEnemyKillReplayFollowupHint(tutorial: TutorialRuntimeState): void {
    const key = "tutorial:first-enemy-kill-replay-followup-hint";
  ...
  function freezeTutorialFirstEnemyKillReplayFollowupHints(): void {
    const key = "tutorial:first-enemy-kill-replay-followup-hint";
    const hintRowKeys = new Set([`${key}:rewind`, `${key}:replay`]);
    let didFreezeHint = false;

    for (const [rowIndex, row] of liveTutorialTimelineRows.entries()) {
      if (row.key === undefined || !hintRowKeys.has(row.key)) {
        continue;
      }
  ...
  function startDebugAiAutorunMode(mode: MultiFactionGameModeId): void {
    ...
    const startWhenCurrentTurnSettles = (): void => {
      if (isTurnTransitionActive || isCommandConsoleResolving) {
        window.setTimeout(startWhenCurrentTurnSettles, 40);
        return;
      }
      ...
      if (selectedMapPreset.procedural === true) {
        resetAiBatchWithAutomaticProceduralMap(mode);
      } else {
        resetDebugAiMode(mode);
      }
    };

    startWhenCurrentTurnSettles();
  }
  ...
  function frameTutorialOpeningCamera(): void {
    ...
    focus.fromArray([...openingCameraPose.focus]);
    renderer.restoreCameraState({
      ...currentCamera,
      focus,
      yaw: openingCameraPose.yaw,
      pitch: openingCameraPose.pitch,
      distance: openingCameraPose.distance,
      focusedTargetKey: openingCameraPose.focusedTargetKey,
      trackedFocusTargetKey: openingCameraPose.trackedFocusTargetKey,
      displayScaleFocusTargetKey: openingCameraPose.displayScaleFocusTargetKey,
      displayScaleDistance: openingCameraPose.displayScaleDistance
    });
  ...
  function createStartStateAuditEvents(
    ...
    const declarations = getStartStateAuditDeclarations(auditState.gameMode).map((declaration) => {
  ...
  function getStartStateAuditDeclarations(mode: GameModeId): readonly StartStateAuditDeclaration[] {
    if (selectedMapPreset.procedural === true && currentProceduralDebug !== null) {
      const starts =
        mode === "1p"
          ? [currentProceduralDebug.playerStart]
          : mode === "3p"
            ? [
                currentProceduralDebug.playerStart,
                currentProceduralDebug.opponentStart,
                currentProceduralDebug.ai_2Start
              ]
            : [currentProceduralDebug.playerStart, currentProceduralDebug.opponentStart];
  ...
  function handleTutorialAfterTurn(
    ...
    if (appendTutorialFirstEnemyKillPostReplayWarning(tutorial)) {
      return;
    }
  ...
  function completeTutorialGuidedSegment(options: Readonly<{ immediate?: boolean }> = {}): void {
    ...
    const timer = window.setTimeout(showCompletionReport, 900);
    tutorial.timers.push(timer);
  }
  ...
  function completeTutorialFirstEnemyKillReplayCue(): void {
    ...
    tutorial.firstEnemyKillReplayWarningPending = true;
    tutorial.inputLocked = false;
    tutorial.autoAdvanceActive = false;
    tutorial.phase = "complete";
    syncTutorialFirstEnemyKillReplayCueLine();
    appendTutorialFirstEnemyKillPostReplayWarning(tutorial);
  }

  function appendTutorialFirstEnemyKillPostReplayWarning(tutorial: TutorialRuntimeState): boolean {
    if (!tutorial.firstEnemyKillReplayWarningPending) {
      return false;
    }

    tutorial.firstEnemyKillReplayWarningPending = false;
    appendTutorialTimelineRows(
      createTutorialEnemyContactVictoryWarningRows(),
      "tutorial:first-enemy-kill-post-replay-warning"
    );
  ...
  tacticalCanvas.addEventListener("pointerdown", (event) => {
    if (isGameMenuDemoActive || currentView !== "tactical2d" || event.button !== 2) {
      return;
    }
  ...
  tacticalCanvas.addEventListener("pointermove", (event) => {
    if (isGameMenuDemoActive || dragStart === null || currentView !== "tactical2d") {
      return;
    }
  ...
  tacticalCanvas.addEventListener("wheel", (event) => {
    if (currentView !== "tactical2d" || isGameMenuDemoActive) {
      if (isGameMenuDemoActive) {
        event.preventDefault();
      }
      return;
  ...
  tacticalCanvas.addEventListener("dblclick", (event) => {
    if (isGameMenuDemoActive || currentView !== "tactical2d") {
      return;
    }
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (isGameMenuDemoActive && event.key.toLowerCase() === "f") {
      event.preventDefault();
      return;
    }
    ...
    if (event.key === "Escape") {
      const selectedNodeId = getNodeIdFromTargetKey(selectedTargetKey);
      const hasSelectedPendingBurn =
        selectedNodeId !== null &&
        snapshot.pendingBurnOrders.some((order) => order.originNodeId === selectedNodeId);
      const hasSelectedPendingFire =
        selectedNodeId !== null &&
        snapshot.pendingFireOrders.some((order) => order.originNodeId === selectedNodeId);

      if (
        hasSelectedPendingFire ||
        snapshot.pendingFireOrders.length > 0 ||
        hasSelectedPendingBurn ||
        snapshot.pendingBurnOrders.length > 0
      ) {
        state = applyCommand(
          state,
          hasSelectedPendingFire && selectedNodeId !== null
            ? {
                type: "CANCEL_PENDING_FIRE_ORDER",
            ...
            : snapshot.pendingFireOrders.length > 0
              ...
              : hasSelectedPendingBurn && selectedNodeId !== null
                ? {
                    type: "CANCEL_PENDING_BURN_ORDER",
                    originNodeId: selectedNodeId
                  }
      ...
      cinematicRenderer?.clearFocus();
      fitSystem();
      sfxEngine.play("ui.cancel");
      redraw();
    }
  });
