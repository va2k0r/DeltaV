...
describe("Cinematic 3D architecture boundary", () => {
  ...
  it("advances cached tactical dash phases at render cadence", () => {
    ...
    expect(source).toContain("flightPath === null ? visiblePoints : points");
  ...
  it("keeps the main HUD clean and exposes the debug drawer only by explicit opt-in", () => {
    ...
    expect(styles).toContain("right: clamp(4px, 0.65vw, 14px)");
    ...
    expect(styles).toContain("--command-console-line-right-gutter: clamp(24px, 3.2vw, 84px)");
  ...
  it("starts 1 PLAYER debug mode as an intradiegetic tutorial", () => {
    ...
    expect(uiSource).toContain("opponent: 50");
    ...
    expect(uiSource).toContain("order.originNodeId === tutorial.shipyardLessonNodeId");
    expect(uiSource).toContain("transit.originNodeId === tutorial.shipyardLessonNodeId");
    ...
    expect(rendererSource).toContain("assist.destinationTargetKey, to");
    ...
    expect(tutorialCameraSource).toContain("focus.fromArray([...openingCameraPose.focus])");
    expect(tutorialCameraSource).toContain("renderer.restoreCameraState");
    expect(tutorialCameraSource).toContain("yaw: openingCameraPose.yaw");
    expect(tutorialCameraSource).toContain("pitch: openingCameraPose.pitch");
    expect(tutorialCameraSource).toContain("distance: openingCameraPose.distance");
  ...
  it("keeps lighting and background ownership in the cinematic renderer", () => {
    ...
    expect(source).toContain("sunSmoothGlareMinimalPerformanceScaleMultiplier");
    expect(source).toContain("sunSmoothGlareMinimalPerformanceOpacityMultiplier");
    expect(source).toContain("sunSmoothGlareSolarStressRadiusStartRatio");
    expect(source).toContain("sunSmoothGlareSolarStressHiddenThreshold");
    ...
    expect(source).not.toContain("float broadFlow =");
    expect(source).not.toContain("float filamentFlow =");
    expect(source).toContain("float gaussianRing =");
    expect(source).toContain("float mirroredField =");
    expect(source).toContain("float solarCell(");
    expect(source).toContain("float mergeDistance =");
    expect(source).toContain("float yellowCells =");
    expect(source).toContain("float surfaceIntensity = min(intensity, 1.02)");
    expect(source).toContain("vec3 amberCore = vec3(1.03, 0.92, 0.24)");
    expect(source).toContain("vec3 pearlWhite = vec3(1.06, 1.05, 1.0)");
    expect(source).toContain("float pearlPlasma =");
    expect(source).toContain("vec3 solarYellowSurface =");
    expect(source).toContain("float baseSolarYellowing =");
    ...
    expect(source).toContain("vec3 edgeWhite = vec3(1.0)");
    ...
    expect(source).toContain("vec3 amberCore =");
    expect(source).toContain("vec3 pearlWhite =");
    expect(source).toContain("vec3 limbHeat =");
  ...
  it("keeps beat sync cached per frame and presentation-only", () => {
    ...
    expect(source).toContain("!this.isBeatSyncEnabled() || this.isReducedPerformanceMode()");
    ...
    expect(source).toContain("performanceStartupReducedHoldMs");
    expect(source).toContain(
      'private adaptivePerformanceMode: Exclude<CinematicPerformanceMode, "auto"> = "reduced"'
    );
    expect(source).not.toContain('this.adaptivePerformanceMode = "full"');
    ...
    expect(source).toContain("rendererPixelRatioHugeViewportFullMax");
    ...
    expect(trajectoryDetailModeSource).toContain(
    ...
    expect(source).toContain("workEffectsReducedUpdateSeconds");
    ...
    expect(source).toContain("contestedThrusterJetsReducedUpdateSeconds");
    ...
    expect(source).toContain("solarDustReducedUpdateSeconds");
    expect(source).toContain("solarDustMinimalUpdateSeconds");
  ...
  it("keeps FIRE presentation explicit and visually separate from BURN", () => {
    ...
    expect(source).toContain("getActiveMissileTargetOrbitAngle");
    ...
    expect(source).toContain("getMissileDefenseInterceptionFlightProgress");
    ...
    expect(source).toContain("missileEvadeInterceptionExplosionScale = 0.6");
    expect(source).toContain(
    ...
    expect(source).toContain("function getMissileEvadeTracerInterceptionPosition");
    expect(source).toContain("function getMissileEvadeInterceptionDelaySeconds");
    expect(source).toContain("function getMissileEvadeTracerTargetShotImpactSeconds");
    expect(source).toContain(
      "const detonationDelaySeconds = getMissileEvadeInterceptionDelaySeconds("
    );
    expect(source).toContain(
    ...
    expect(source).toContain("interceptionProgress");
  ...
  it("suppresses mouseover text throughout the tutorial without disabling hover previews", () => {
    const rendererSource = readFileSync(
      join(process.cwd(), "src/renderers/cinematic3d/index.ts"),
      "utf8"
    );
  ...
  it("keeps selected burn destination affordances visual-only", () => {
    ...
    expect(source).toContain('variant: "default" | "own-ship-present"');
    expect(source).toContain('text === "OWN SHIP PRESENT" ? "own-ship-present" : "default"');
  ...
  it("keeps full quality outside trailer mode and adaptive quality inside it", () => {
    const uiSource = readFileSync(join(process.cwd(), "src/ui/index.ts"), "utf8");
    ...
    expect(modeResolverSource).toContain('return isTrailerModeActive ? "auto" : "full";');
  ...
  it("toggles a non-destructive in-game menu with Escape and Resume", () => {
    const uiSource = readFileSync(join(process.cwd(), "src/ui/index.ts"), "utf8");
    ...
    expect(renderMenuSource.indexOf('"RESUME"')).toBeLessThan(
      renderMenuSource.indexOf('"TUTORIAL"')
    );
  ...
  it("keeps tutorial command log wheel inert while rewind is disabled", () => {
    ...
    expect(wheelSource).toContain("if (isTutorialCommandLogLocked()) {");
    expect(wheelSource).toContain("event.preventDefault();");
    expect(wheelSource).toContain("event.stopPropagation();");
    expect(wheelSource).toContain("return;");
    expect(wheelSource.indexOf("if (isTutorialCommandLogLocked()) {")).toBeLessThan(
      wheelSource.indexOf("normalizeCommandLogWheelDelta(")
    );
    expect(uiSource).toContain(
  ...
  it("provides paced 1/2/3 command-log transport and an exact first-turn scroll stop", () => {
    ...
    expect(uiSource).toContain("beginTrailerReplayLogRewrite");
    expect(uiSource).toContain("syncTrailerReplayCommandLog(clampedPosition)");
    expect(uiSource).toContain("getReplayEntryVisualProgress");
    expect(styles).toContain(".is-trailer-replay-future");
    expect(styles).toContain("@keyframes command-trailer-replay-reveal");
  });
  ...
  it("coalesces timeline previews into the renderer frame and caches replay-only work", () => {
    ...
    const logSyncStart = uiSource.indexOf("  function syncTrailerReplayCommandLog(");
    const logSyncEnd = uiSource.indexOf("  function endTrailerReplayLogRewrite(", logSyncStart);
    const logSyncSource = uiSource.slice(logSyncStart, logSyncEnd);
    ...
    expect(uiSource).toContain("rebuildTrailerReplayLogCache()");
    expect(logSyncSource).toContain("trailerReplayLogLineCache");
    expect(logSyncSource).not.toContain("querySelectorAll");
    expect(logSyncSource).not.toContain("offsetWidth");
    expect(logSyncSource).not.toContain("setTimeout");
    expect(uiSource).toContain("handleTrailerReplayLogRevealAnimationEnd");
    expect(uiSource).toContain(
    ...
    expect(uiSource).toContain('return isTrailerModeActive ? "auto" : "full"');
    expect(rendererSource).toContain("const performanceFrameBudgetReducedMs = 8.55");
    expect(rendererSource).toContain("const performanceFrameBudgetMinimalMs = 11.5");
  ...
  it("reconstructs ship destruction and orbital wreckage in both replay directions", () => {
    ...
    expect(rendererSource).toContain("return impactTarget.center.clone();");
  ...
  it("keeps replay playback presentation-only and camera-stable", () => {
    ...
    expect(uiSource).not.toContain(
      "void animateCommandLogTimeReviewToPosition(replayTape.transitions.length, null)"
    );
    ...
    expect(globalTimeReviewPointerSource).not.toContain("skipCommandLogTimeReviewToLive");
    ...
    expect(uiSource).toContain("Left click the blinking log line to rewind to that point in time.");
    expect(uiSource).toContain("Left click the blinking log line again to resume.");
    expect(uiSource).toContain(
  ...
  it("keeps occupied-node selection and burn confirmation visually distinct", () => {
    ...
    const getActiveMissileTargetOrbitAngleStart = source.indexOf(
      "  private getActiveMissileTargetOrbitAngle(",
      getFireTargetRenderDataStart
    );
    const getFireTargetRenderDataSource = source.slice(
      getFireTargetRenderDataStart,
      getActiveMissileTargetOrbitAngleStart
    );
    ...
    expect(createPreviewBurnLaunchOriginSource).toContain(
      "this.getRenderedShipMarkerWorldPosition(plan.originNodeId, factionId)"
    );
    expect(createPreviewBurnLaunchOriginSource).toContain(
