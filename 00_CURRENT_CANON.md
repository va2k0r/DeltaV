...
4. Ship arrivals / Intercept resolution
5. Actions: Work eligibility, Fire, Burn, Intercept orders
6. Economy:
   a. Tritium income
...
```text
A contested ship cannot Work, Fire, Evade, or Intercept.
...
Ships outside contested may Fire into the contested node or Intercept reinforcements/escapes if position and range rules allow.
...
Intercept = hard movement denial
Contested = present physical lock
```

With missile minimum travel time 2 and local ship travel time 1, Burn/Intercept/Contested remain more important than pure missile spam.
...
Intercept creates a temporary contested intercept node.

```text
On successful Intercept:
- target transfer is cancelled
- interceptor and target enter a temporary contested intercept node
- no Work is possible there
- the node disappears when one ship Burns out or is destroyed
```

## 9. Shipyard progress is stealable
...
## 10. Map canon v10: planets and moons only
...
## 12. Gas giant + shipyard in same system
...
Moons are tactical ground: Fire, Intercept, staging, contested support.
```

## 15. Canonical visual and color direction
